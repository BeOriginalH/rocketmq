...
public class CommitLog {
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            switch (result.getStatus()) {
                case PUT_OK://添加成功
                    break;
                case END_OF_FILE://文件不足以保存消息，创建一个新的额MappedFile
     ...
     * 首先获取目录下第一个文件的，如果可用，返回该文件的最小偏移量，否则查返回下一个文件的起始偏移量
     *
     * @return
     */
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        /**
         * 存储msgId的ByteBuffer  4字节IP+4字节端口号+8字节消息偏移量
         */
        private final ByteBuffer msgIdMemory;

        /**
         * 存储ipv6的msgId的ByteBuffer 16字节IP+4字节端口号+8字节消息偏移量
         */
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            //生成msgId
            String msgId;
            if ((sysflag & MessageSysFlag.STOREHOSTADDRESS_V6_FLAG) == 0) {//不是使用IPv6
            ...
            if (msgLen > this.maxMessageSize) {//消息长度超过最大值，默认4K
            ...
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {//如果本页写写不下这条消息
            ...
            // 2 MAGICCODE 魔数 固定值daa320a7
            this.msgStoreItemMemory.putInt(CommitLog.MESSAGE_MAGIC_CODE);
            // 3 BODYCRC 消息体crc校验
            this.msgStoreItemMemory.putInt(msgInner.getBodyCRC());
            // 4 QUEUEID 队列ID
            this.msgStoreItemMemory.putInt(msgInner.getQueueId());
            // 5 FLAG 消息Flag，rocketmq不使用，程序可以使用
            this.msgStoreItemMemory.putInt(msgInner.getFlag());
            // 6 QUEUEOFFSET  消息在消息消费队列的偏移量
            this.msgStoreItemMemory.putLong(queueOffset);
            // 7 PHYSICALOFFSET 消息在CommitLog中的偏移量
            this.msgStoreItemMemory.putLong(fileFromOffset + byteBuffer.position());
            // 8 SYSFLAG 消息系统Flag，是否压缩 是否是事务消息
            this.msgStoreItemMemory.putInt(msgInner.getSysFlag());
            // 9 BORNTIMESTAMP 消息生产者调用消息发送API的时间戳
            this.msgStoreItemMemory.putLong(msgInner.getBornTimestamp());
            // 10 BORNHOST 消息发送者的IP、端口号
            this.resetByteBuffer(bornHostHolder, bornHostLength);
            this.msgStoreItemMemory.put(msgInner.getBornHostBytes(bornHostHolder));
            // 11 STORETIMESTAMP 消息存储的时间戳
            this.msgStoreItemMemory.putLong(msgInner.getStoreTimestamp());
            // 12 STOREHOSTADDRESS broker的IP和端口
            this.resetByteBuffer(storeHostHolder, storeHostLength);
